...
class ResNet(nn.Module):
    def __init__(
        ...
        self.aux1 = self._make_layer(block, 64, layers[0])
        self.aux2 = self._make_layer(block, 128, layers[1], stride=2, dilate=replace_stride_with_dilation[0])
        self.aux3 = self._make_layer(block, 256, layers[2], stride=2, dilate=replace_stride_with_dilation[1])
        self.aux4 = self._make_layer(block, 512, layers[3], stride=2, dilate=replace_stride_with_dilation[2])
    ...
    def _forward_impl(self, x, prototypes=None, epoch=None, labels=None):
        ...
        batch_size = x.size()[0]
        y=x
        #rep = rep.permute(0,2,3,1).reshape(batch_size, 1024, 256)
        x = self.layer1(x)
        x = self.layer2(x)
        x = self.layer3(x)
        rep = x.permute(0,2,3,1).reshape(batch_size, 256, 256)
        #x = self.layer2(x)
        #x = self.layer3(x)
        #rep = x.permute(0,2,3,1).reshape(batch_size, 1024, 256)
        x = self.layer4(x)
        if (epoch !=None) and (epoch > 1) and (prototypes != None):
            y = self.layer1(y)
            y = self.layer2(y)
            y = self.layer3(y)
            y =  self.get_augmented_feature(y.permute(0,2,3,1).reshape(batch_size, 256, 256), labels, prototypes, epoch)
            y = y.reshape(batch_size, 16,16, 256).permute(0,3,1,2)
            y = self.layer4(y)
        ...
        if  (prototypes == None) or (epoch==None) or (epoch<=1):
            loss = torch.zeros(batch_size).cuda()
        else:
            target = torch.ones(batch_size).cuda()
            loss = torch.cosine_embedding_loss(x, y,target)
    ...
    def forward(self, x, prototypes=None, epoch=None, labels=None):
        return self._forward_impl(x, prototypes, epoch, labels)
