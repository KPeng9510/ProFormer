...
class OneShotTester(BaseTester):
    ...
    def do_knn_and_accuracies(self, accuracies, embeddings_and_labels, split_name, tag_suffix=''):
        ...
        for i in range(7):
            ...
            reference_embeddings[i,:] = reference_embeddings_1[1*i:1*i+1,:].mean(axis=0)
...
def get_datasets(data_dir, cfg, mode="train"):
    ...
    common_transforms.append(transforms.Resize((384, 384)))
...
def train_app(cfg):
    ...
    #path = '/home/kpeng/oneshot_metriclearning/transformer-sl-dml/outputs/2021-10-12/11-27-41/example_saved_models_c_2_cat/swin_PMBFA_toyota_24_7/'
    #path = '/home/kpeng/oneshot_metriclearning/transformer-sl-dml/outputs/2021-10-12/08-54-12/example_saved_models_c_2_cat/levit_PMBFA_toyota_24_7/'
    #path = '/home/kpeng/oneshot_metriclearning/transformer-sl-dml/outputs/2021-10-26/09-26-31/example_saved_models_c_2_cat/pat/'
    #path = '/home/kpeng/oneshot_metriclearning/transformer-sl-dml/outputs/toyota_swin/'
    path = '/home/kpeng/oneshot_metriclearning/sl-dml/outputs/2021-10-12/07-26-56/example_saved_models/resnet_one_shot_toyota_24_7/'
    embedder = MLP([trunk_output_size, cfg.embedder.size])
    embedder.load_state_dict(torch.load(path+'embedder_best4.pth'))
    embedder = torch.nn.DataParallel(embedder.to(device)).eval()
    classifier = MLP([cfg.embedder.size, 24])
    classifier.load_state_dict(torch.load(path+'classifier_best4.pth'))
    classifier = torch.nn.DataParallel(classifier.to(device)).eval()
    trunk.load_state_dict(torch.load(path + 'trunk_best4.pth'))
