...
class OneShotTester(BaseTester):
    ...
    def do_knn_and_accuracies(self, accuracies, embeddings_and_labels, split_name, classifier_model, tag_suffix='',):
        ...
        query_prediction = torch.argmax(torch.nn.functional.softmax(classifier_model(torch.from_numpy(embeddings_and_labels["val"][0]).cuda()), dim=-1), dim=-1).cpu().numpy()
        ...
        accuracy = accuracy_score(query_prediction, query_labels)
        print(accuracy)
...
def train_app(cfg):
    ...
    classifier = torch.nn.DataParallel(MLP([cfg.embedder.size, 120])).to(device)

    # Set optimizers
    if cfg.optimizer.name == "sdg":
        trunk_optimizer = torch.optim.SGD(trunk.parameters(), lr=cfg.optimizer.lr, momentum=cfg.optimizer.momentum,
                                          weight_decay=cfg.optimizer.weight_decay)
        embedder_optimizer = torch.optim.SGD(embedder.parameters(), lr=cfg.optimizer.lr,
                                             momentum=cfg.optimizer.momentum, weight_decay=cfg.optimizer.weight_decay)
        classifier_optimizer = torch.optim.SGD(classifier.parameters(), lr=cfg.optimizer.lr,
                                               momentum=cfg.optimizer.momentum, weight_decay=cfg.optimizer.weight_decay)
    elif cfg.optimizer.name == "rmsprop":
        trunk_optimizer = torch.optim.RMSprop(trunk.parameters(), lr=cfg.optimizer.lr, momentum=cfg.optimizer.momentum,
                                              weight_decay=cfg.optimizer.weight_decay)
        embedder_optimizer = torch.optim.RMSprop(embedder.parameters(), lr=cfg.optimizer.lr,
                                                 momentum=cfg.optimizer.momentum,
                                                 weight_decay=cfg.optimizer.weight_decay)
        classifier_optimizer = torch.optim.RMSprop(classifier.parameters(), lr=cfg.optimizer.lr,
                                                   momentum=cfg.optimizer.momentum,
                                                   weight_decay=cfg.optimizer.weight_decay)
    elif cfg.optimizer.name == 'adam':
        trunk_optimizer = torch.optim.Adam(trunk.parameters(), lr=cfg.optimizer.lr,
                                           weight_decay=cfg.optimizer.weight_decay)
        embedder_optimizer = torch.optim.Adam(embedder.parameters(), lr=cfg.optimizer.lr,
                                              weight_decay=cfg.optimizer.weight_decay)
        classifier_optimizer = torch.optim.Adam(classifier.parameters(), lr=cfg.optimizer.lr,
                                                weight_decay=cfg.optimizer.weight_decay)

    # Set the datasets
    data_dir = os.environ["DATASET_FOLDER"]
    print("Data dir: " + data_dir)

    train_dataset, val_dataset = get_datasets(data_dir, cfg, mode=cfg.mode.type)
    print("Trainset: ", len(train_dataset), "Testset: ", len(val_dataset))
    ...
    if cfg.embedder_loss.name == "multi_similarity":
        loss = losses.MultiSimilarityLoss(alpha=cfg.embedder_loss.alpha, beta=cfg.embedder_loss.beta,
                                          base=cfg.embedder_loss.base)
    ...
    schedulers = {
        ...
        "embedder_scheduler_by_epoch": torch.optim.lr_scheduler.StepLR(embedder_optimizer, 10,
                                                                       gamma=cfg.scheduler.gamma),
        "classifier_scheduler_by_epoch": torch.optim.lr_scheduler.StepLR(classifier_optimizer, 10,
                                                                         gamma=cfg.scheduler.gamma),
    ...
    experiment_name = "model_c2_cat_levit_%s_model_%s_cl_%s_ml_%s_miner_%s_mix_ml_%02.2f_mix_cl_%02.2f_resize_%d_emb_size_%d_class_size_%d_opt_%s_lr_%02.2f_m_%02.2f_wd_%02.2f" % (
    cfg.dataset.name,
    ...
    record_keeper, _, _ = logging_presets.get_record_keeper("logs_c_2_cat/%s" % (experiment_name),
                                                            "tensorboard_c_2_cat/%s" % (experiment_name))
