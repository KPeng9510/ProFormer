...
class OneShotTester(BaseTester):
    ...
    def do_knn_and_accuracies(self, accuracies, embeddings_and_labels, split_name, tag_suffix=''):
        ...
        reference_embeddings = np.zeros((12, 128))
        #print(reference_embeddings_1.shape)
        #sys.exit()
        for i in range(12):
            #mask = reference_labels_1 == i+100
            #mask = np.squeeze(mask)
            reference_embeddings[i,:] = reference_embeddings_1[5*i:5*i+5,:].mean(axis=0)
        reference_labels = np.arange(0,20)

        #reference_labels = embeddings_and_labels["samples"][1]
        knn_indices, knn_distances = utils.stat_utils.get_knn(reference_embeddings.astype('float32'), query_embeddings.astype('float32'), 1, False)
        ...
        for i in range(12):
            ...
            query_embeddings = np.concatenate([query_embeddings, reference_embeddings_1[3 * i+1:3 * i + 3, :]], axis=0)
    ...
    def do_knn_and_accuracies_aug(self, accuracies, embeddings_and_labels, split_name, tag_suffix=''):
        ...
        knn_indices, knn_distances = utils.stat_utils.get_knn(reference_embeddings.astype(np.float32), query_embeddings,
                                                              1, False)
        knn_labels = reference_labels[knn_indices][:, 0]
        accuracy = accuracy_score(knn_labels, query_labels)
        print(accuracy)
...
def get_datasets(data_dir, cfg, mode="train"):
    ...
    if cfg.transform.transform_random_rotation:
        train_transforms.append(
            transforms.RandomRotation(cfg.transform.transform_random_rotation_degrees))  # , fill=255))
    ...
    if mode == "final_train":
        ...
        samples_dataset = datasets.ImageFolder(os.path.join(data_dir, "samples"),
                                               data_transforms["test"])
        return test_dataset, samples_dataset
    else:
        if mode == "train":
            ...
            val_samples_dataset = datasets.ImageFolder(os.path.join(data_dir, "val_samples"),
                                                       data_transforms["test"])
            return val_dataset, val_samples_dataset
...
def train_app(cfg):
    ...
    # trunk = LeViT_384_cascaded(num_classes=512, distillation=True,
    #          pretrained=False, fuse=False)
    ...
    #trunk = build_model()
    trunk_output_size = 768 #trunk.fc.in_features
    trunk.head = Identity()
    trunk.head_2 = Identity()
    path ='/home/kpeng/oneshot_metriclearning/transformer-sl-dml/outputs/2021-10-25/14-41-33/example_saved_models_c_2_cat/levit60_rep_twostage__proPMBFA_re_nturgbd_dataset_100_20_noise/' #levit60twostage
    #path = "/home/kpeng/oneshot_metriclearning/transformer-sl-dml/outputs/2021-10-01/20-05-33/example_saved_models_c_2_cat/Swin_retrain_PMBFA/"
    #path_resnet = "/home/kpeng/oneshot_metriclearning/sl-dml/outputs/2021-09-27/11-38-48/example_saved_models/resnet18_baseline_one_shot_nturgbd120/"
    #path = '/home/kpeng/oneshot_metriclearning/models/output_transformer_dml/2021-09-21/13-34-59/example_saved_models_c_2_cat/model_c2_cat_levit_NTU_ONE_SHOT_SWAP_AXIS_model_resnet18_cl_cross_entropy_ml_triplet_margin_miner_multi_similarity_mix_ml_0.50_mix_cl_0.50_resize_256_emb_size_128_class_size_21_opt_rmsprop_lr_0.00_m_0.00_wd_0.00/'
    #path_levit = '/home/kpeng/oneshot_metriclearning/transformer-sl-dml/outputs/2021-10-01/20-15-35/example_saved_models_c_2_cat/Levit_retrain_PMBFA/'
    #path_levit_pmbfa = '/home/kpeng/oneshot_metriclearning/transformer-sl-dml/outputs/2021-10-09/07-52-58/example_saved_models_c_2_cat/levit_re_PMBFA_nturgbd60_48_12/'
    #path_ = '/home/kpeng/oneshot_metriclearning/transformer-sl-dml/outputs/2021-10-08/17-09-26/example_saved_models_c_2_cat/Swin_re_PMBFA_nturgbd60_48_12/'

    #path = '/home/kpeng/oneshot_metriclearning/transformer-sl-dml/outputs/2021-10-09/20-45-17/example_saved_models_c_2_cat/swin_PMBFA_nturgbd60_48_12/'
    embedder = MLP([trunk_output_size, cfg.embedder.size])
    embedder.load_state_dict(torch.load(path+'embedder_best23.pth'))
    embedder = torch.nn.DataParallel(embedder.to(device)).eval()
    classifier = MLP([cfg.embedder.size, 49])
    classifier.load_state_dict(torch.load(path+'classifier_best23.pth'))
    classifier = torch.nn.DataParallel(classifier.to(device)).eval()
    trunk.load_state_dict(torch.load(path + 'trunk_best23.pth'))
    trunk = torch.nn.DataParallel(trunk.to(device)).eval()
    # Set the datasets
    data_dir = os.environ["DATASET_FOLDER"] + "/" + cfg.dataset.data_dir
    print("Data dir: " + data_dir)

    val_dataset, val_samples_dataset = get_datasets(data_dir, cfg, mode=cfg.mode.type)
    ...
    dataset_dict = {"samples": val_samples_dataset, "val": val_dataset}
    model_folder = "example_saved_models_c__cat/%s/" % (experiment_name)
    # Create the tester
    tester = OneShotTester(
        end_of_testing_hook=hooks.end_of_testing_hook,

    )
    tester.test(dataset_dict=dataset_dict,epoch=1,trunk_model=trunk, embedder_model=embedder, classifier_model=classifier, splits_to_eval=['samples','val'])
