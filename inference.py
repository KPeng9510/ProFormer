...
class TemporalDownSample(object):
    ...
    def __init__(self, downsample_rate=14):
...
class OneShotTester(BaseTester):
    ...
    def do_knn_and_accuracies(self, accuracies, embeddings_and_labels, split_name, tag_suffix=''):
        ...
        reference_embeddings = np.zeros((20, 128))
        #print(reference_embeddings_1.shape)
        #sys.exit()
        for i in range(20):
             #mask = reference_labels_1 == i+100
        #    #mask = np.squeeze(mask)
            reference_embeddings[i,:] = reference_embeddings_1[7*i:7*i+7,:].mean(axis=0)
        ...
        #conf = confusion_matrix(query_labels, knn_labels, reference_labels)
        ...
        #logging.info('confusion_matrix:{}'.format(conf))
        logging.info('accuracy:{}'.format(accuracy))
        logging.info('f_1_score:{}'.format(f_1_score))
        logging.info('precision:{}'.format(precision))
        logging.info('recall:{}'.format(recall))
        #print(accuracy)
        with open(self.embedding_filename + "_last_ms_levit", 'wb') as f:
...
def train_app(cfg):
    ...
    #trunk = build_model()
    trunk_output_size = 512 #trunk.fc.in_features
    #trunk.fc = Identity()
    ...
    #path = "/home/kpeng/oneshot_metriclearning/sl-dml/outputs/2021-09-27/11-38-48/example_saved_models/resnet18_baseline_one_shot_nturgbd120/"
    ...
    #path = '/home/kpeng/oneshot_metriclearning/transformer-sl-dml/outputs/2021-10-01/20-15-35/example_saved_models_c_2_cat/Levit_retrain_PMBFA/'
    #path = '/home/kpeng/oneshot_metriclearning/transformer-sl-dml/outputs/2021-10-27/20-29-10/example_saved_models_c_2_cat/levit_100_20_noise_0.09/'
    embedder = MLP([trunk_output_size, cfg.embedder.size])
    embedder.load_state_dict(torch.load(path+'embedder_best90.pth'))
    embedder = torch.nn.DataParallel(embedder.to(device)).eval()
    classifier = MLP([cfg.embedder.size, 21])
    classifier.load_state_dict(torch.load(path+'classifier_best90.pth'))
    classifier = torch.nn.DataParallel(classifier.to(device)).eval()
    trunk.load_state_dict(torch.load(path + 'trunk_best90.pth'))
