...
import sys
import hydra
from gcn_augmentation import Padding
...
def get_datasets(data_dir, cfg, mode="train"):
    ...
    data_transforms = {
            'train': transforms.Compose([Padding()]),
            'test': transforms.Compose(Padding()),
            }
...
def train_app(cfg):
    #cfg = {}
    #for item,val in zip(cfg1.keys(),cfg1.values()):
    #    cfg[item]=val[str(item)]
    print(cfg.pretty())
    ...
    trunk = Model() #LeViT_384(num_classes=512, distillation=True,
    #          pretrained=False, fuse=False)
