...
class ResT(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.patch_embed_4_2 = PatchEmbed(patch_size=2, in_ch=embed_dims[2], out_ch=embed_dims[3], with_pos=True)
        # transformer encoder
        ...
        self.head = nn.Linear(embed_dims[3], 512) if num_classes > 0 else nn.Identity()
        self.head_2 = nn.Linear(embed_dims[3], 512) if num_classes > 0 else nn.Identity()
        self.cosine_embedding_loss = torch.nn.CosineEmbeddingLoss(margin=0.0, size_average=None, reduce=None, reduction='mean')
        self.f_feature = nn.Linear(256, 128)
        self.f_prototypes = nn.Linear(256, 128)
        self.softmax = nn.Softmax()
        self.f_a = nn.Linear(128*2, 128)
        self.f_r = nn.Linear(128, 256)
        self.relu = nn.ReLU()
        # init weights
    ...
    def get_augmented_feature(self, features, labels, prototypes):
        ...
        B,H,C = curbatch_proto.size()
        #curbatch_proto = torch.nn.functional.upsample_bilinear(curbatch_proto.reshape(B,4,4,C).permute(0,3,1,2), scale_factor=4.0)
        #print(curbatch_proto.size())
        lift_proto = self.f_prototypes(curbatch_proto.reshape(B,256,24*24))
        #print(lift_proto.size())

        #features = nn.functional.max_pool2d(features, kernel_size=(4,4)).reshape(B,C,H*W).permute(0,2,1)
        lift_feature = self.f_feature(features)
        #print(lift_feature.size())
        ...
        #aug_feature = torch.nn.functional.upsample_bilinear(self.relu(features + self.f_r(out_phi_a)).permute(0,2,1).reshape(B,256,4,4), scale_factor=4.0)
        return (self.relu(features + self.f_r(out_phi_a)).permute(0,2,1).reshape(B,24,24,C).permute(0,3,1,2))
    def forward(self, x, prototypes=None, epoch=None, labels=None):
        ...
        if (prototypes == None) or (epoch==1) or (epoch == None):

            for blk in self.stage3:
                x = blk(x, H, W)
            rep = x
            x = x.permute(0, 2, 1).reshape(B, -1, H, W) # 8,246,24,24
            #rep = nn.functional.max_pool2d(x, kernel_size =(4,4)).reshape(B,-1, H*W).permute(0,1,2)
            ...
            for blk in self.stage3:
                x = blk(x, H, W)
            x_ori = x.permute(0,2,1)
            rep = x.permute(0,2,1)
            x = x.permute(0, 2, 1).reshape(B, -1, H, W) # 8,246,24,24
            #rep = nn.functional.max_pool2d(x, kernel_size=(4,4)).reshape(B,-1, H*W).permute(0,1,2)

            x_aug = self.get_augmented_feature(x_ori, labels, prototypes)
            #print(rep.size()) # 8,246,6,6
            #print(x.size())
            # stage 4
            x_aug, (h_a, w_a) = self.patch_embed_4_2(x_aug)
            x, (H, W) = self.patch_embed_4(x)
            for blk in self.stage4:
                x = blk(x, H, W)
            for blk in self.stage4_2:
                x_aug = blk(x_aug, h_a, w_a)
            x = self.norm(x)
            # print(x.size())
            x = x.permute(0, 2, 1).reshape(B, -1, H, W)
            x = self.avg_pool(x).flatten(1)
            ...
            # print(x.size())
            x_aug = x_aug.permute(0, 2, 1).reshape(B, -1, H, W)
            x_aug = self.avg_pool(x_aug).flatten(1)
            #x_aug = self.head_2(x_aug)
            target = torch.ones(x_aug.size()[0]).cuda()
            loss = torch.cosine_embedding_loss(x, x_aug,target)
