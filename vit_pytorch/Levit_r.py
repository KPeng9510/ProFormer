...
class LeViT(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        for block in self.blocks:
            x = block(x)
            if count in self.intermediate_mark:
                #print(count)
                inter.append(x)
            count+=1

        #print(inter[0].size(), inter[1].size())
        #sys.exit()
        inter1 = inter[1]
        #inter = self.head_inter(inter_1)
        #inter_cls = self.fuse(inter_1)
        #x = self.blocks(x)
        ...
        #print(x.size())
        #print(inter.size())
        #x=self.merge(torch.cat([x, inter], dim=-1))
        return x, inter1
class MLP(nn.Module):
...
class LeViT_cascaded(torch.nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    ...
    def forward(self, x):
        ...
        inter = []
        for block in self.blocks:
            if count > self.intermediate_mark[1]:
                x=block(x)
            count += 1
            #print(count)
        # print(inter[0].size(), inter[1].size())
